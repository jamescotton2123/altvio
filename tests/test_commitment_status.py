...
class FakeSupabase:
    def __init__(self, docusign_status: str):
        self.docusign_status = docusign_status
        self.billing_usage = []
                ...
                return [
                    {
                        ...
                        "docusign_status": self.db.docusign_status,
                        "funded_amount": "0",
                    }
                ]
            return []
...
def _materialize_for_status(docusign_status: str):
    fake_supabase = FakeSupabase(docusign_status=docusign_status)
    ...
    billing.materialize_billing_period("firm-1", "2026-05", granularity="monthly")
    onboarding_rows = [
        row for row in fake_supabase.billing_usage
        if row["event_type"] == "onboarding_complete"
    ]
    return billing, onboarding_rows


@pytest.mark.parametrize("docusign_status", ["Signed", "Completed"])
def test_onboarding_fee_fires_for_signed_and_completed_docusign_states(docusign_status):
    billing, onboarding_rows = _materialize_for_status(docusign_status)

    assert len(onboarding_rows) == 1
    assert onboarding_rows[0]["amount_cents"] == billing.ONBOARDING_FEE_CENTS


@pytest.mark.parametrize("docusign_status", ["Pending", "Sent"])
def test_onboarding_fee_does_not_fire_for_incomplete_docusign_states(docusign_status):
    _, onboarding_rows = _materialize_for_status(docusign_status)

    assert onboarding_rows == []
